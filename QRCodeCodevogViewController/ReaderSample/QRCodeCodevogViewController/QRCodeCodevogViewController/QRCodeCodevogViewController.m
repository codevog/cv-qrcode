...
- (void) addForms
{
    ...
    [_textField setFrame:CGRectMake(0, 0, 0, 35)];//height
    ...
    // [view addConstraint:[NSLayoutConstraint constraintWithItem:view attribute:NSLayoutAttributeWidth relatedBy:NSLayoutRelationEqual toItem:_viewPreview attribute:NSLayoutAttributeHeight multiplier:1.5 constant:0.0]];
...
{
    int indent = 90;
    if ([[UIDevice currentDevice].model hasPrefix:@"iPad"])//(UI_USER_INTERFACE_IDIOM() != UIUserInterfaceIdiomPhone)
    {
        indent = 169; //338/2
    }
    
    //    CGRect rectFrame =  CGRectMake(self.view.frame.size.width/2 - indent, _viewPreview.frame.origin.y + (_viewPreview.frame.size.height/2-indent), 2*indent, 2*indent);
...
-(void)captureOutput:(AVCaptureOutput *)captureOutput didOutputMetadataObjects:(NSArray *)metadataObjects fromConnection:(AVCaptureConnection *)connection{
    ...
    if (metadataObjects != nil && [metadataObjects count] > 0) {
        ...
        if ([[metadataObj type] isEqualToString:AVMetadataObjectTypeQRCode]) {
            ...
            //            if ([metadataObj stringValue].length == 10)
            //            {
            //                [self performSelectorOnMainThread:@selector(stopReading) withObject:nil waitUntilDone:NO];
            //                [self performSelectorOnMainThread:@selector(setTextToDataLabel:) withObject:[metadataObj stringValue] waitUntilDone:YES];
            //            _isReading = NO;
            //            }
            if (_audioPlayer) {
                [_audioPlayer play];
...
- (void) codeFound
{
    ...
    if (_doneImage)
    {
        //UIImage *done = [UIImage imageNamed:@"done"];
