...
{
    //[[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(QRCodeValue) name:@"QRCodeReceived" object:nil];
    //QRCodeReceived
    QRCodeCodevogViewController *code = [[QRCodeCodevogViewController alloc] init];
    code.doneImage = [UIImage imageNamed:@"done"];
    [self presentViewController:code animated:NO completion:nil];
}

- (void) QRCodeValue: (NSString *) code
{
